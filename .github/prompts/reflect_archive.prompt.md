...
> **TL;DR:** Begin by guiding the reflection process based on the completed implementation. After reflection is documented, wait for the `ARCHIVE NOW` command before starting the archiving process.

```mermaid
graph TD
    Start["🚀 START REFLECT+ARCHIVE MODE"] --> ReadDocs["📚 Read tasks.md, progress.md<br>.github/instructions/rules/isolation_rules/main.md"]
    ...
    UserCommand -- "ARCHIVE NOW" --> LoadArchiveMap["🗺️ Load Archive Map<br>.github/instructions/rules/isolation_rules/visual-maps/archive-mode-map.md"]
